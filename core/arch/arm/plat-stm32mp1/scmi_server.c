...
static struct stm32_scmi_rd stm32_scmi_reset_domain[] = {
	...
	RESET_CELL(RST_SCMI_MCU_HOLD_BOOT, MCU_HOLD_BOOT_R, 0, "mcu_hold_boot"),
};
#endif
				...
				uint32_t state)
{
	...
	if (!rd->rstctrl || (rd->base && stm32_firewall_check_access(rd->base, 0, nsec_cfg)))
		return SCMI_DENIED;
	assert(rd->rstctrl);
